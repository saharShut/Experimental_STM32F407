  ...
  else if (xcpInfo.connected == 1)
  {
    switch (data[0])
    {
      ...
      case XCP_CMD_GET_STATUS:
        // ILI9341_WriteString(0, 40, "XCP_CMD_GET_STATUS", Font_11x18, ILI9341_BLACK, ILI9341_WHITE);
      ...
      case XCP_CMD_PROGRAM_CLEAR:
        // ILI9341_WriteString(0, 60, "XCP_CMD_PROGRAM_CLEAR", Font_11x18, ILI9341_BLACK, ILI9341_WHITE);
...
static void XcpCmdProgramClear(blt_int8u *data)
{
  ...
  char par[64];
  xsprintf(par, "Address = %X, len = %X", (uint32_t)eraseAddr, eraseLen);
  ILI9341_WriteString(0, 60, par, Font_11x18, ILI9341_BLACK, ILI9341_WHITE);
  if (NvmErase(eraseAddr, eraseLen) == BLT_FALSE)
  {
    /* error occurred during erasure */
    ILI9341_WriteString(0, 80, "XCP_ERR_GENERIC", Font_11x18, ILI9341_BLACK, ILI9341_WHITE);
    XcpSetCtoError(XCP_ERR_GENERIC);
    return;
  }
